...
class Database_loader :
    ...
    def get_next_train(self, random_flip_flop = False, random_rotate = False, verbose = False) :
...
#        print('crop ', box)
    ...
    def get_next_train_batch(self, batch_size = 50, random_flip_flop = False, random_rotate = False) :

        batch_image = np.empty([batch_size, self.size, self.size, self.nb_channels])
        batch_label = np.empty([batch_size, self.nb_class])
        for i in range(0,batch_size) :
            data = self.get_next_train(random_flip_flop,random_rotate, verbose=False)
    ...
    def get_next_test(self, random_flip_flop = False, random_rotate = False, verbose = False) :
    ...
    def get_batch_test(self, batch_size = 50, random_flip_flop = False, random_rotate = False) :
        
        batch_image = np.empty([batch_size, self.size, self.size, self.nb_channels])
        batch_label = np.empty([batch_size, self.nb_class])
        for i in range(0,batch_size) :
            data = self.get_next_test(random_flip_flop,random_rotate)
            batch_image[i] = data[0]
            batch_label[i] = data[1]
    ...
    def get_next_validation(self, random_flip_flop = False, random_rotate = False, verbose = False) :
    ...
    def get_batch_validation(self, batch_size = 50, random_flip_flop = False, random_rotate = False) :
        
        batch_image = np.empty([batch_size, self.size, self.size, self.nb_channels])
        batch_label = np.empty([batch_size, self.nb_class])
        for i in range(0,batch_size) :
            data = self.get_next_validation(random_flip_flop,random_rotate, verbose=False)
            batch_image[i] = data[0]
            batch_label[i] = data[1]
...
if __name__ == "__main__":    
    ...
    a = Database_loader('/home/nicolas/Documents/Stage 3A/Test', 300, only_green=True)
